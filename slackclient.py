...
class SlackClient:
    ...
    def _post(self, channel: str, payload: dict):
        ...
        if self.debug:
            ...
            try:
                r = requests.post(self.webhook_url, data=str(payload).encode("utf-8"))
                self.logger.debug(f'Response from slack: {r.status_code}')
                self.logger.debug(f'Content: {r.content}')
